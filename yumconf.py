...
from common import VALID_BASEARCHES, VALID_DVERS, Error, to_bytes, to_str
...
class YumInstallError(Error):
    def __init__(self, packages, rootdir, err):
        super(self.__class__, self).__init__(
            "Could not install %r into %r (rpm/yum process returned %d)"
            % (packages, rootdir, err)
        )
...
class YumDownloaderError(Error):
    def __init__(self, packages, rootdir, err, resolve=False):
        super(self.__class__, self).__init__(
            "Could not download %r into %r (resolve=%s) (yumdownloader process returned %d)"
            % (packages, rootdir, resolve, err)
        )
...
class YumEraseError(Error):
    def __init__(self, packages, rootdir, err):
        super(self.__class__, self).__init__(
            "Could not erase %r from %r (rpm process returned %d)"
            % (packages, rootdir, err)
        )
...
class YumInstaller:
    ...
    def _get_repo_args(self):
        ...
        for sec in self.config.sections():
            if self.config.has_option(sec, 'enabled') and self.config.getboolean(
                sec, 'enabled'
            ):
    ...
    def _add_repos(self):
        self.repotemplate = ConfigParser.SafeConfigParser(
            {'dver': self.dver, 'basearch': self.basearch}
        )
    ...
    def repoquery(self, *args):
        ...
        if type(args[0]) is list or type(args[0]) is tuple:
            args = list(args[0])
        cmd = ["repoquery", "-c", self.conf_file.name]
    ...
    def force_erase(self, installroot, packages):
        ...
        cmd = ["rpm", "--erase", "--verbose", "--nodeps", "--root", installroot]
        cmd += packages
        env = os.environ.copy()
        env.update({'LANG': 'C', 'LC_ALL': 'C'})
        err = subprocess.call(cmd, env=env)
